...
import com.meng.bilibilihelper.fragment.LoginCoinFragment;
import com.meng.bilibilihelper.fragment.MainFrgment;
import com.meng.bilibilihelper.fragment.ManagerFragment;
import com.meng.bilibilihelper.fragment.NaiFragment;
import com.meng.bilibilihelper.fragment.SignFragment;
...
public class MainActivity extends Activity {
    ...
    private void showDialogTipUserRequestPermission() {
        ...
        new AlertDialog.Builder(this)
                .setTitle("权限申请")
                .setMessage("本软件需要网络权限和存储权限用于连接屑站服务器和部分数据存储")
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == 321) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (grantResults[0] != PackageManager.PERMISSION_GRANTED && !shouldShowRequestPermissionRationale(permissions[0])) {
                    showToast("缺失存储权限会使应用工作不正常");
                }
                if (grantResults[1] != PackageManager.PERMISSION_GRANTED && !shouldShowRequestPermissionRationale(permissions[1])) {
                    showToast("缺失网络权限会使应用工作不正常");
                }
            }
        }
    }
