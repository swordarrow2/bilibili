                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            for (LoginInfoPeople loginInfoPeople : MainActivity.instence.loginInfo.loginInfoPeople) {
                                try {
                                    Thread.sleep(1000);
                              //      Tools.BilibiliTool.sendLiveDanmaku(MainActivity.instence.getFragment(NaiFragment.class).getRandomSentense(), loginInfoPeople.cookie, MainActivity.instence.getFragment("Main",MainFragment.class).mengEditText.getLiveId());
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            for (LoginInfoPeople loginInfoPeople : MainActivity.instence.loginInfo.loginInfoPeople) {
                                try {
                                    Thread.sleep(1000);
                                    MainActivity.instence.getFragment("签到",SignFragment.class).sendSignData(loginInfoPeople.cookie);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
